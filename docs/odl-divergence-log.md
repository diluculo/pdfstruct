...
| followup | Implemented 2026-05-02. `AssignHeadingLevels` no longer clamps with `Math.Min(level, 6)` — the data model carries uncapped integer levels. The existing `"level"` JSON field was repurposed to carry the ODL-parity vocabulary (`Doctitle` at level 1, `Subtitle` at level 2 and below) instead of adding a parallel `"level name"` field; this keeps the JSON one-key-one-meaning and direct-diffable against ODL output. The Markdown renderer was already clamping at render time with `Math.Clamp(level, 1, 6)` so no renderer change was required. Baseline-MD diff is empty; baseline-JSON `"level"` strings updated on the four fixtures that previously emitted heading elements. |
